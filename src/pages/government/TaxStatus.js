...
class TaxStatusPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      drivers: [],
  ...
  onToggleStatus = async (id, tax_status) => {
    let params = {}
    if(tax_status === 'current'){
      params = {
        id,
                        ...
                        <tbody>
                          {
                            search_drivers.map((item, index) => (
                              <tr key={index}>
                                <th scope="row"><div className={item.alert?"red-circle":""}>{index + 1}</div></th>
                                ...
                                <td> <Button
                                    onClick={() => this.onToggleStatus(item.id, item.tax_status)}
                                    color="primary"
                                    className="mx-2"
                                >
                                  {" "}
